...
Func UpdateBotTitle()
	Local $sTitle = "My Bot " & $g_sBotVersion & " - " & "Demen MOD" & " " & $g_sModversion & " -"
...
Func InitializeBot()
	...
	SetSwitchAccLog(_PadStringCenter(" SwitchAcc LOG ", 25, "="), $COLOR_BLACK, "Lucida Console", 8, False) ; Switch Accounts
...
Func FinalInitialization(Const $sAI)
	...
	UpdateMultiStats() ; Switch Accounts
...
Func runBot() ;Bot that runs everything in order
	...
	While 1
		...
		If $g_bIsClientSyncError = False And $g_bIsSearchLimit = False And ($g_bQuickAttack = False) Then
			...
			VillageReport()
			CheckStopForWar()
			UpdateHeroStatus() ; Hero and Lab Status
			UpdateLabStatus() ; Hero and Lab Status
			ProfileSwitch() ; Switch Profile
			CheckFarmSchedule() ; Farm Schedule
...
Func _Idle() ;Sequence that runs until Full Army
	...
	While $g_bIsFullArmywithHeroesAndSpells = False
		...
		If $g_ichkUseBotHumanization = 1 Then BotHumanization() ; Bot Humanization
