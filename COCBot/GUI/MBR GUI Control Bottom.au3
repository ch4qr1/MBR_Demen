...
Func Initiate()
	...
	If $g_hAndroidWindow <> 0 And ($g_bAndroidBackgroundLaunched = True Or AndroidControlAvailable()) Then
		...
		$g_bFirstStart = True
		$g_bInitiateSwitchAcc = True ; Switch Accounts
...
Func btnPause($bRunNow = True)
	TogglePause()
	GUICtrlSetState($g_hBtnDisableGUI, $GUI_HIDE) ; Enable/Disable GUI while botting
	GUICtrlSetState($g_hBtnEnableGUI, $GUI_SHOW) ; Enable/Disable GUI while botting
	; Stop on Low battery
...
Func btnResume()
	TogglePause()
	GUICtrlSetState($g_hBtnDisableGUI, $GUI_HIDE) ; Enable/Disable GUI while bottin
	GUICtrlSetState($g_hBtnEnableGUI, $GUI_HIDE) ; Enable/Disable GUI while botting
	; Stop on Low battery
	GUICtrlSetState($g_hLblBatteryAC, $GUI_SHOW)
	GUICtrlSetState($g_hLblBatteryStatus, $GUI_SHOW)
EndFunc   ;==>btnResume

; Enable/Disable GUI while botting
Func btnEnableGUI()
