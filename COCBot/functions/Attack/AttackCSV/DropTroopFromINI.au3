...
Func DropTroopFromINI($vectors, $indexStart, $indexEnd, $indexArray, $qtaMin, $qtaMax, $troopName, $delayPointmin, $delayPointmax, $delayDropMin, $delayDropMax, $sleepafterMin, $sleepAfterMax, $sleepBeforeMin, $sleepBeforeMax, $debug = False)
	...
	Else
		...
		EndIf
		$sleepBefore = Int($sleepBefore / $g_CSVSpeedDivider[$g_iMatchMode]); CSV Deploy Speed - Demen MOD (#-09)
		...
		For $i = $indexStart To $indexEnd
			...
			If $index <> $indexMax Then
				...
				$delayDrop = Int($delayDrop / $g_CSVSpeedDivider[$g_iMatchMode]); CSV Deploy Speed - Demen MOD (#-09)
			...
			For $j = 1 To $numbersOfVectors
				...
				If $index <= UBound(Execute("$" & Eval("vector" & $j))) Then
					...
					If $delayPointmin <> $delayPointmax Then
						...
						$delayPoint = Int($delayPoint / $g_CSVSpeedDivider[$g_iMatchMode]); CSV Deploy Speed - Demen MOD (#-09)
					Else
					...
					Switch $iTroopIndex
						...
						Case $eKing
							...
							Else
								dropHeroes($pixel[0], $pixel[1], $troopPosition, -1, -1) ; was $g_iKingSlot, Demen - ExtendedAttackBar - Demen MOD (#-22)
							EndIf
						Case $eQueen
							If $debug = True Then
								SetLog("dropHeroes(" & $pixel[0] & ", " & $pixel[1] & ",-1," & $g_iQueenSlot & ", -1) ")
							Else
								dropHeroes($pixel[0], $pixel[1], -1, $troopPosition, -1) ; was $g_iQueenSlot, Demen - ExtendedAttackBar - Demen MOD (#-22)
							EndIf
						Case $eWarden
							If $debug = True Then
								SetLog("dropHeroes(" & $pixel[0] & ", " & $pixel[1] & ", -1, -1," & $g_iWardenSlot & ") ")
							Else
								dropHeroes($pixel[0], $pixel[1], -1, -1, $troopPosition) ; was $g_iWardenSlot, Demen - ExtendedAttackBar - Demen MOD (#-22)
							EndIf
						Case $eCastle
							If $debug = True Then
								SetLog("dropCC(" & $pixel[0] & ", " & $pixel[1] & ", " & $g_iClanCastleSlot & ")")
							Else
								dropCC($pixel[0], $pixel[1], $troopPosition) ; was $g_iClanCastleSlot, Demen - ExtendedAttackBar - Demen MOD (#-22)
							EndIf
		...
		EndIf
		$sleepafter = Int($sleepafter / $g_CSVSpeedDivider[$g_iMatchMode]); CSV Deploy Speed - Demen MOD (#-09)
