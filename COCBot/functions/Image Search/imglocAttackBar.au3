			...
			For $i = 0 To UBound($aResult) - 1
				...
				If $aResult[$i][1] > 0 Then
					...
					Else
						...
						$aResult[$i][4] = -1
					EndIf
					If $aResult[$i][4] <= 10 Or Not $g_abChkExtendedAttackBar[$g_iMatchMode] Or Not $SWIPE = "" Then ; ExtendedAttackBar - Demen MOD (#-22)
						$strinToReturn &= "|" & TroopIndexLookup($aResult[$i][0]) & "#" & $aResult[$i][4] & "#" & $aResult[$i][3] & "#" & $aResult[$i][1] ; SWIPE - Demen MOD (#-33)
					EndIf
				EndIf
			Next
		EndIf
